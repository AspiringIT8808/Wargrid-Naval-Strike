...
# One button per ship ability, then the optional Repair / Shield.
...
class PlayScreen(Screen):
    def __init__(self, app):
        super().__init__(app)
        self.view = 0                       # whose fleet is shown on the left
        self.pass_pending = False           # duo: turn is over, waiting to hide the screen
        self.selected = None                # selected targeted ability / repair / shield
        self.horizontal = True              # Destroyer orientation while relocating
        self.msg = ""
        self.ai_next = 0                    # time when the computer may act again
    ...
    def action_state(self, a):
        """(enabled, second line of text) for one action button."""
    ...
    def prompt(self):
        ...
        hints = {None: "ATTACK (click enemy waters) OR use ONE ability",
    ...
    def click_board(self, pos):
        ...
        if sel is None:
            ...
            if enemy:
                self.do(lambda: g.use_ability(SHIP_BY_KEY[sel], enemy if sel == "recon" else enemy[0]))
        elif sel == "relocate":
            if own:
    ...
    def after_action(self, who):
        ...
        elif g.current != who:                              # the turn passed
            if self.app.mode == "duo":
                self.pass_pending = True                    # let the player see the result first
            elif g.current == 1:
    ...
    def pass_device(self):
        g = self.game
        lines = [f"{g.names[g.current]}'s turn", ""] + g.last_turn_log[-3:] + \
    ...
    def draw(self):
        ...
        text(self.prompt(), (50, 598), FONT, ACCENT if self.can_act() else ORANGE)
        text(self.msg, (50, 624), SMALL, TEXT)

        if self.pass_pending:                               # duo: "pass the device" button
            self.pass_btn.draw(label=f"END TURN  -  pass to {g.names[g.current]}  (SPACE)")
        else:
            can_start = self.can_act() and g.attacks_left == 0
            for a, btn in zip(ACTIONS, self.action_btns):
                enabled, sub = self.action_state(a)
                btn.draw(enabled and can_start, self.selected == a["id"], sub, a["label"])
        text("ESC / right-click cancels a selection.   Ability turns end your turn; only the Submarine also shoots.",
             (50, 735), SMALL, MUTED)

    def draw_previews(self, mine, theirs):
        """Highlights that show what the next click would do."""
        ...
        if enemy and sel is None and theirs.can_fire(enemy):
            ...
            highlight(area_cells(enemy, RECON_SIZE), RIGHT_X, BOARD_Y, (244, 218, 102, 90))
        elif enemy and sel == "row_scan":
            highlight([(enemy[0], c) for c in range(GRID)], RIGHT_X, BOARD_Y, (244, 218, 102, 90))
        elif sel == "relocate" and own:
            ship = mine.get_ship(SHIP_BY_KEY[sel])
            cells = line_cells(own, ship.size, self.horizontal)
            color = good if mine.check_move(ship, cells) is None else (230, 78, 92, 110)
            highlight([c for c in cells if in_bounds(c)], LEFT_X, BOARD_Y, color)
        elif sel == "repair":
            highlight(mine.repairable_cells(), LEFT_X, BOARD_Y, (244, 218, 102, 90))
        elif sel == "shield":
            highlight(mine.shieldable_cells(), LEFT_X, BOARD_Y, (90, 200, 240, 70))
